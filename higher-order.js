...
//Code Here
let postTaxPrices = prices.map(a => a += a * 0.07);
...
//Code Here
let totalPopulation = populations.reduce((a, v) => a + v);
...
let orderTotals = orders.map(a => a = a["price"] + (a["price"] * a["tax"]));
...
let bobsTotal = purchases.reduce((a, v) => {
  return v["owner"] === "Bob" ? a + v["price"] : a;
}, 0);